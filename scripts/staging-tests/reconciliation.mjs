...
export function evaluateFinalState({
  ...
  initialCatalogFingerprint, finalCatalogFingerprint,
}) {
  const failures = [];
  ...
  const currentSessions = new Set(current.sessionIds);
  if (baseline.sessionIds.some(id => !currentSessions.has(id))) failures.push("BASELINE_SESSION_REMOVED");
  if (runSessionIds.some(id => currentSessions.has(id))) failures.push("RUN_SESSION_REMAINS");
...
export async function reconcileRun({ db, baselines, actorIds, runSessionIds = {}, markers, voucherIds, initialCatalogFingerprint, recovery = false }) {

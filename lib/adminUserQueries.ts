...
async function summarizeUsers(userIds: string[], latest: Map<string, Date>, now: Date): Promise<AdminUserSummary[]> {
  if (userIds.length === 0) return [];
  const { year, start, end } = vietnamYearBounds(now);
  const [users, spend, spent, exchanged, current] = await Promise.all([
    prisma.user.findMany({ where: { id: { in: userIds }, role: "CUSTOMER" }, select: {
      ...
      is_verified: true, is_blocked: true, points_balance: true,
    } }),
  ...
  return userIds.flatMap((id) => {
    ...
    return [{
      ...
      insta_name: user.insta_name, is_verified: user.is_verified, is_blocked: user.is_blocked,
      ...
      current_voucher_count: currentMap.get(id) ?? 0, latest_order_at: latest.get(id)?.toISOString() ?? null,
    }];
  });
}

/** Lists CUSTOMER accounts by latest order, followed by accounts with no orders. */
export async function listAdminUsers(page: number, q?: string, now = new Date()): Promise<AdminUserPage<AdminUserSummary>> {
  ...
  const [total, orderedCount] = await Promise.all([
    prisma.user.count({ where }),
    prisma.user.count({ where: { AND: [where, { orders: { some: {} } }] } }),
  ]);
  const orderedSkip = Math.min(offset, orderedCount);
  const orderedTake = Math.min(PAGE_SIZE, Math.max(0, orderedCount - offset));
  const grouped = orderedTake > 0 ? await prisma.order.groupBy({
    by: ["user_id"], where: { user_id: { not: null }, user: { is: where } },
    _max: { created_at: true }, orderBy: [{ _max: { created_at: "desc" } }, { user_id: "asc" }],
    skip: orderedSkip, take: orderedTake,
  }) : [];
  const orderedIds = grouped.flatMap((row) => row.user_id ? [row.user_id] : []);
  const latest = new Map(grouped.flatMap((row) => row.user_id && row._max.created_at ? [[row.user_id, row._max.created_at] as const] : []));
  const remaining = PAGE_SIZE - orderedIds.length;
  const neverOrdered = remaining > 0 ? await prisma.user.findMany({
    where: { AND: [where, { orders: { none: {} } }] }, select: { id: true },
    orderBy: { qr_token: "asc" }, skip: Math.max(0, offset - orderedCount), take: remaining,
  }) : [];
  const ids = [...orderedIds, ...neverOrdered.map((user) => user.id)];
  return { items: await summarizeUsers(ids, latest, now), total, page, total_pages: Math.ceil(total / PAGE_SIZE) };
}
...
export async function getAdminUser(userQrToken: string, now = new Date()): Promise<AdminUserSummary | null> {
  ...
  if (!user) return null;
  const latestOrder = await prisma.order.findFirst({ where: { user_id: user.id }, select: { created_at: true }, orderBy: [{ created_at: "desc" }, { id: "asc" }] });
  return (await summarizeUsers([user.id], new Map(latestOrder ? [[user.id, latestOrder.created_at]] : []), now))[0] ?? null;
}
...
export async function listAdminUserVoucherPackages(page: number, category: AdminUserVoucherCategory, now = new Date()): Promise<AdminUserPage<AdminUserVoucherPackage>> {
  const voucherTypes: VoucherType[] | undefined = category === "DISCOUNT" ? ["DISCOUNT"] : category === "SHIPPING" ? ["FREESHIP"]
    : category === "GIFT" ? ["ITEM", "PRODUCT", "PRODUCT_DISCOUNT", "ADDON", "BUNDLE"] : undefined;
...
const adminUserOrderInclude = {
  ...
  bundleApplications: { include: {
    ...
    rewards: { select: {
      ...
      orderItemAddon: { select: { addonOption: { select: { label: true } } } },
    } },
  } },
  ...
  return {
    ...
    bundle_applications: row.bundleApplications.map((application) => ({
      ...
      rewards: application.rewards.map((reward) => ({
        order_item_id: reward.order_item_id, addon_label: reward.orderItemAddon?.addonOption.label ?? null,

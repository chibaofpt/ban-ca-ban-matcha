...
export default function AdminVoucherPackagesPage() {
  ...
  const menuItems = [...(menu?.latte ?? []), ...(menu?.fusion ?? [])];
  const menuOptions = menuItems.map((item) => ({ value: item.id, label: item.name, description: item.is_seasonal ? "Theo mùa" : item.category === "latte" ? "Latte" : "Fusion" }));

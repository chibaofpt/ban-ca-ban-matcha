...
const TYPE_LABEL: Record<VoucherPackage["voucher_type"], string> = {
  BUNDLE: "Mua X tặng Y", PRODUCT: "Sản phẩm", ADDON: "Addon",
...
export default function AdminVoucherPackagesPage() {
  ...
  const menuOptions = [...(menu?.latte ?? []), ...(menu?.fusion ?? [])].map((item) => ({ value: item.id, label: item.name, description: item.is_seasonal ? "Theo mùa" : item.category === "latte" ? "Latte" : "Fusion" }));
  ...
  const menuPriceById = new Map([...(menu?.latte ?? []), ...(menu?.fusion ?? [])].map((item) => [item.id, Math.max(0, ...item.sizes.map((size) => size.base_price_vnd ?? 0))]));
  ...
  return <main className="mx-auto w-full max-w-6xl space-y-5 px-4 py-6 pb-28 md:px-8">
    ...
    <VoucherWizard open={wizardOpen} onOpenChange={setWizardOpen} menuOptions={menuOptions} addonOptions={addonOptions} powderOptions={powderOptions} milkOptions={milkOptions} menuPriceById={menuPriceById} addonPriceById={addonPriceById} submitting={createMutation.isPending} onSubmit={submit} />

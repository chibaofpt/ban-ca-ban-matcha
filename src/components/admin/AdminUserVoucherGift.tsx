...
const categories: Array<{ value: AdminUserVoucherCategory; label: string }> = [{ value: "ALL", label: "Tất cả" }, { value: "DISCOUNT", label: "Discount" }, { value: "GIFT", label: "Tặng món" }, { value: "SHIPPING", label: "Shipping" }];
const voucherTypeText: Record<string, string> = { DISCOUNT: "Giảm toàn đơn", PRODUCT_DISCOUNT: "Giảm món", ITEM: "Tặng món", PRODUCT: "Tặng sản phẩm", ADDON: "Tặng topping", BUNDLE: "Combo", FREESHIP: "Shipping" };
...
function PackageRow({ pkg, pending, onGift }: { pkg: AdminUserVoucherPackage; pending: boolean; onGift: () => void }) { return <li className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="break-words font-medium">{pkg.name}</p>{pkg.description ? <p className="break-words text-sm text-muted-foreground">{pkg.description}</p> : null}<p className="mt-1 text-xs font-medium text-primary">{voucherTypeText[pkg.voucher_type] ?? pkg.voucher_type}</p></div><Button type="button" size="sm" disabled={pending} aria-label={`Tặng voucher ${pkg.name}`} onClick={onGift} className="min-h-11 shrink-0 gap-1"><Gift className="h-4 w-4" aria-hidden="true" />Tặng</Button></li>; }

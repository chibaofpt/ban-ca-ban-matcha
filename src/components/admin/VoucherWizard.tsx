...
function BenefitFields({ draft, update, menuOptions, addonOptions, powderOptions, milkOptions }: {
  draft: VoucherDraft; update: <K extends keyof VoucherDraft>(key: K, value: VoucherDraft[K]) => void;
  menuOptions: AdaptiveSelectOption[]; addonOptions: AdaptiveSelectOption[];
  powderOptions: AdaptiveSelectOption[]; milkOptions: AdaptiveSelectOption[];
}) {
  if (draft.voucherType === "BUNDLE") return <div className="space-y-4">
    <div className="grid grid-cols-2 gap-3"><NumberField label="Mua X món" value={draft.buyQuantity} min={1} onChange={(value) => update("buyQuantity", value ?? 1)} /><NumberField label="Tặng Y phần" value={draft.rewardQuantity} min={1} onChange={(value) => update("rewardQuantity", value ?? 1)} /></div>
    <AdaptiveSelect label="Nhóm món được tính điều kiện" multiple options={menuOptions} value={draft.qualifierMenuItemIds} onChange={(value) => update("qualifierMenuItemIds", value as string[])} />
    ...
    {draft.rewardKind === "PRODUCT" ? <>
      <AdaptiveSelect label="Cách xác định cấu hình quà" options={[{ value: "SAME_CONFIG", label: "Cùng loại và cấu hình món mua" }, { value: "FIXED_CONFIG", label: "Cấu hình cố định" }, { value: "ALLOWED_SCOPE", label: "Trong nhóm, miễn đến hạn mức" }]} value={draft.rewardMode} onChange={(value) => update("rewardMode", value as VoucherDraft["rewardMode"])} />
      {draft.rewardMode !== "SAME_CONFIG" ? <AdaptiveSelect label="Nhóm món có thể nhận quà" multiple options={menuOptions} value={draft.rewardMenuItemIds} onChange={(value) => update("rewardMenuItemIds", value as string[])} /> : null}
      {draft.rewardMode === "FIXED_CONFIG" ? <div className="grid gap-3 sm:grid-cols-3"><AdaptiveSelect label="Size quà" options={[{ value: "SMALL", label: "Small" }, { value: "MEDIUM", label: "Medium" }, { value: "LARGE", label: "Large" }]} value={draft.rewardSize} onChange={(value) => update("rewardSize", value as VoucherDraft["rewardSize"])} /><AdaptiveSelect label="Bột quà" options={powderOptions} value={draft.rewardPowderId} onChange={(value) => update("rewardPowderId", value as string)} /><AdaptiveSelect label="Sữa quà (nếu có)" options={[{ value: "", label: "Mặc định" }, ...milkOptions]} value={draft.rewardMilkTypeId} onChange={(value) => update("rewardMilkTypeId", value as string)} /></div> : null}
      {draft.rewardMode === "ALLOWED_SCOPE" ? <NumberField label="Hạn mức mỗi quà (VND)" value={draft.referencePriceVnd} min={1_000} step={1_000} onChange={(value) => update("referencePriceVnd", value ?? 0)} /> : null}
    </> : <><AdaptiveSelect label="Nhóm addon có thể nhận quà" multiple options={addonOptions} value={draft.rewardAddonOptionIds} onChange={(value) => update("rewardAddonOptionIds", value as string[])} /><AdaptiveSelect label="Cách nhân quyền lợi addon" options={[{ value: "PER_BUNDLE", label: "Theo mỗi nhóm mua X" }, { value: "ONCE_PER_ORDER", label: "Một lần trong đơn" }, { value: "PER_QUALIFYING_ITEM", label: "Trên mỗi món đủ điều kiện" }]} value={draft.benefitScaling} onChange={(value) => update("benefitScaling", value as VoucherDraft["benefitScaling"])} /></>}
    {draft.benefitScaling !== "ONCE_PER_ORDER" ? <NumberField label="Số nhóm tối đa trong một đơn" value={draft.maxApplications} min={1} onChange={(value) => update("maxApplications", value ?? 1)} /> : null}
    <NumberField label="Giá trị đơn tối thiểu (để trống nếu không cần)" value={draft.minOrderVnd} min={1_000} step={1_000} onChange={(value) => update("minOrderVnd", value)} />
  </div>;
...
export function VoucherWizard(props: VoucherWizardProps) {
  ...
  const review = describeVoucherDraft(draft, menuLabels, addonLabels); const liability = estimateVoucherLiabilityVnd(draft, props.menuPriceById, props.addonPriceById);
  ...
  {step === 2 ? <BenefitFields draft={draft} update={update} menuOptions={props.menuOptions} addonOptions={props.addonOptions} powderOptions={props.powderOptions} milkOptions={props.milkOptions} /> : null}

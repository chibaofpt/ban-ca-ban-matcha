...
export function VoucherIssuanceFields({ draft, update, form, submitting, review, liability }: VoucherIssuanceFieldsProps) {
  ...
  const points = draft.acquisitionMode === "POINTS_EXCHANGE" && draft.visibility === "PUBLIC" ? draft.pointsCost : 0;
  ...
  const visibilityControls = <fieldset className="space-y-2"><legend className="text-sm font-semibold">Phạm vi hiển thị</legend><div className="grid min-w-0 grid-cols-2 gap-2"><button type="button" disabled={submitting} aria-pressed={draft.visibility === "PUBLIC"} onClick={() => update("visibility", "PUBLIC")} className={`min-h-14 rounded-xl border px-3 py-2 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${draft.visibility === "PUBLIC" ? "border-primary bg-primary/10 text-primary" : "border-input"}`}>Công khai</button><button type="button" disabled={submitting} aria-pressed={draft.visibility === "PRIVATE"} onClick={() => update("visibility", "PRIVATE")} className={`min-h-14 rounded-xl border px-3 py-2 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${draft.visibility === "PRIVATE" ? "border-primary bg-primary/10 text-primary" : "border-input"}`}>Riêng tư</button></div></fieldset>;
  if (draft.visibility === "PRIVATE") return <div className="min-w-0 space-y-4 overflow-x-clip">{visibilityControls}<p className="rounded-xl bg-muted p-3 text-sm text-muted-foreground">Gói riêng tư chỉ được admin tích chọn để tặng trực tiếp cho khách. Không có luồng tự nhận, đổi điểm hoặc tự động cấp.</p><div className="grid min-w-0 grid-cols-1 gap-3"><NumberField label="Tổng phát hành" value={draft.quantity} min={1} onBlur={() => validateField("quantity")} onChange={(value) => update("quantity", value)} error={errorFor("quantity")} /></div><div className="grid min-w-0 grid-cols-[minmax(0,7fr)_minmax(0,3fr)] gap-3"><label className="block min-w-0 space-y-1.5"><span className="text-sm font-semibold">Ngày kết thúc</span><input type="date" value={draft.endsAt} onBlur={() => validateField("endsAt")} onChange={(event) => update("endsAt", event.target.value)} className={inputClass} /></label><NumberField label="Hạn dùng sau khi tặng" value={draft.expiresAfterDays} min={1} onBlur={() => validateField("expiresAfterDays")} onChange={(value) => update("expiresAfterDays", value)} error={errorFor("expiresAfterDays")} /></div><div className="rounded-xl bg-muted p-4 text-sm"><strong>{draft.name}</strong><p className="mt-1 text-muted-foreground">{review}</p></div></div>;

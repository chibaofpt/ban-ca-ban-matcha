...
/** Manage draft campaign box images and normalized mouth anchors. */
export function RewardBoxEditor({ campaign, saving, onCreate, onUpdate, onDelete }: RewardBoxEditorProps) {
  ...
  return (
    <section className="space-y-4" aria-labelledby="reward-box-title">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 id="reward-box-title" className="font-bold">Hộp matcha</h3><p className="text-sm text-muted-foreground">{campaign.box_count}/12 hộp · cần 3–12 hộp để kích hoạt</p></div><Button variant="outline" disabled={!editable || campaign.box_count >= 12} onClick={() => setTarget("new")}><Plus className="size-4" /> Thêm hộp</Button></div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {campaign.boxes.map((box) => (
            <article key={box.id} className="rounded-2xl border bg-card p-3">
              ...
              <div className="mt-3 flex items-center justify-between gap-2"><div><h4 className="font-semibold">{box.name}</h4><p className="text-xs text-muted-foreground">Anchor {box.mouth_anchor_x}, {box.mouth_anchor_y}</p></div>{editable ? <div className="flex"><Button variant="ghost" size="icon" aria-label={`Sửa ${box.name}`} onClick={() => setTarget(box)}><Pencil className="size-4" /></Button><Button variant="ghost" size="icon" aria-label={`Xóa ${box.name}`} onClick={() => setDeleteTarget(box)}><Trash2 className="size-4 text-destructive" /></Button></div> : null}</div>
            </article>
          ))}
        </div>
      )}
      ...
      <ResponsiveOverlay open={target !== null} title={current ? `Sửa ${current.name}` : "Thêm hộp matcha"} description="Cấu hình ảnh và vị trí miệng hộp." size="md" layer="critical" onOpenChange={(open) => { if (!open && !saving) setTarget(null); }}>
        {target ? <RewardBoxForm key={current?.id ?? "new"} current={current} revision={campaign.revision} saving={saving} onCancel={() => setTarget(null)} onCreate={onCreate} onUpdate={onUpdate} /> : null}
      </ResponsiveOverlay>
